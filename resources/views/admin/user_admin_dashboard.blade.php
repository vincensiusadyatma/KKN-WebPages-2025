...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
         @foreach ($list_users as $user)
              <a href="{{ route('show-admin-detail', $user->id) }}" class="group bg-white rounded-xl shadow hover:shadow-lg transition p-5 border border-gray-100 hover:border-indigo-400">
            <div class="flex items-center gap-4">
                <img src="{{ asset('img/profile1.png') }}" alt="Avatar" class="w-14 h-14 rounded-full">

                <div class="flex-1">
                    <h3 class="text-lg font-semibold text-gray-900 group-hover:text-indigo-600">{{ $user->username }}</h3>
