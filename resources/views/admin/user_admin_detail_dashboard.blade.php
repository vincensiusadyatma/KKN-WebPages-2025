...
@php
    $statusColor = match($user->status) {
        ...
        default => 'bg-yellow-100 text-yellow-700', // untuk pending atau lainnya
    };
@endphp
...
<div class="flex items-center space-x-6">
    ...
    <div class="mt-6 space-y-4">
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2 mt-1">
                <span class="px-3 py-1 bg-purple-100 text-purple-700 text-xs rounded-full">{{ $user->roles->first()?->name }}</span>
            </div>
        </div>
    ...
    <form action="{{ route('change-admin-status', $user->email) }}" method="POST" class="inline-block">
        ...
        <button type="submit" class="px-5 py-2 bg-green-600 text-white text-sm font-semibold rounded-md hover:bg-green-700 transition">
            Setujui Permintaan
        </button>
    </form>
    ...
    <form action="{{ route('change-admin-status', $user->email) }}" method="POST" class="inline-block ml-3">
        ...
        <button type="submit" class="px-5 py-2 bg-red-600 text-white text-sm font-semibold rounded-md hover:bg-red-700 transition">
            Tolak Permintaan
        </button>
    </form>
</div>
